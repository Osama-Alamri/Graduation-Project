...
def update_password(user_id, current_password, new_password):
    ...
    # 2. Verify the 'current_password' input
    if not verify_password(current_password, current_hash):
        conn.close()
        return False, "Incorrect current password."
        
    # 3. If correct, hash and update the new password
    try:
            ...
            if selected_title:
                ...
                if not applications:
                    ...
                    if selected_app_name:
                        ...
                        # --- This is inside the "View Applications" page logic ---
                                    ...
                                    try:
                                        ...
                                        if isinstance(questions, list) and isinstance(answers, list) and len(questions) == len(answers):
                                            ...
                                            for i, (q, a) in enumerate(zip(questions, answers)):
                                                ...
                                                st.info(q) # Blue box for question
                                                st.markdown(f"**Candidate's Answer:**")
                                                st.text(a) # Simple text for answer
                                                st.divider()
...
# --- This is the new Account Page ---
elif selected_page == "Account":
    st.header("Account Management")

    # This page is only visible if logged in, but we double-check
    if not st.session_state['logged_in']:
        st.error("You must be logged in to view this page.")
    else:
        st.subheader("Change Your Password")
        
        # We use clear_on_submit=True so the fields are empty after success
...
# --- This should be the last page left ---
elif selected_page in ["View Applications"]:
    # (This assumes you already built View Applications, 
    # if not, keep its placeholder logic here)
